...
if len(sys.argv) < 5:
    print("n5a: Needs 5 arguments - FEN string, search type (depth or time), search length, engine location and number of threads")
    sys.exit()

engine = chess.engine.SimpleEngine.popen_uci(sys.argv[4])
engine.configure({"Threads": int(sys.argv[5])})
with engine.analysis(chess.Board(str(sys.argv[1]))) as analysis:
    for info in analysis:
        print(info, end=' ', flush=True)

        # Arbitrary stop condition.
        if info.get(sys.argv[2], 0) > int(sys.argv[3]):
            break
...
engine.quit()'''
    ...
    for resource in exe.pip_install(["chess"]):
